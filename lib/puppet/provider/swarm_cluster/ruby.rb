  ...
  def swarm_conf
    ...
    case
      ...
      when cluster.match(/join/)
        [['--experimental', 'join', "--advertise=#{bind}:2375", "#{backend}://#{address}:#{port}/#{path}"]]
      when cluster.match(/manage/)
        [['--experimental', 'manage', '-H', "tcp://#{bind}:2376", "#{backend}://#{address}:#{port}/#{path}"], ['--experimental', 'manage', '-H', ':4000', '--replication', '--advertise', "#{advertise}:4000", "#{backend}://#{address}:#{port}/#{path}"]]
      end
   end
